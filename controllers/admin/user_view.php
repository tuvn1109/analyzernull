...
if($plugins->exists_and_active('twitch')) {
    $sql .= " UNION SELECT 'twitch' AS `source`, `unlocked_reports`.`date`, `unlocked_reports`.`expiration_date`, `twitch_users`.`username`, `twitch_users`.`full_name` as `full_name` FROM `unlocked_reports` LEFT JOIN `twitch_users` ON `unlocked_reports`.`source_user_id` = `twitch_users`.`id` WHERE `unlocked_reports`.`source` = 'FACEBOOK' AND `user_id` = {$user_id}";
}
...
if($plugins->exists_and_active('twitch')) {
    $sql .= " UNION SELECT 'twitch' AS `source`, `twitch_users`.`full_name`, `twitch_users`.`username` FROM `twitch_users` LEFT JOIN `favorites` ON `favorites`.`source_user_id` = `twitch_users`.`id` WHERE `favorites`.`source` = 'FACEBOOK' AND `favorites`.`user_id` = {$user_id}";
}
